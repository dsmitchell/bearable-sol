...
interface IBearable {
    ...
    /// @dev Throws if the token ID is not valid.
    function bearBottomColor(uint256 tokenId) external view returns (ISVGTypes.Color memory color);
    ...
    /// @dev Throws if the token ID is not valid.
    function bearTopColor(uint256 tokenId) external view returns (ISVGTypes.Color memory color);
}
